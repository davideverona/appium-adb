...
/**
 * Retrieve the list of devices visible to adb (long).
 *
 * @return {Array.<Device>} The list of devices or an empty list if
 *                          no devices are connected.
 * @throws {Error} If there was an error while listing devices.
 */
systemCallMethods.getConnectedDevicesLong = async function getConnectedDevicesLong () {
  log.debug('Getting connected devices long');
  let stdout;
  try {
    ({stdout} = await exec(this.executable.path, [...this.executable.defaultArgs, 'devices', '-l']));
  } catch (e) {
    throw new Error(`Error while getting connected devices. Original error: ${e.message}`);
  }
  const listHeader = 'List of devices';
  ...
  const devices = stdout.split('\n')
    ...
    .reduce((acc, line) => {
      // state is "device", afaic
      let [udid, state, product, model, device, transport_id] = line.split(/\s+/);
      product = product.replace('product:', '');
      model = model.replace('model:', '');
      device = device.replace('device:', '');
      transport_id = transport_id.replace('transport_id:', '');
      acc.push({udid, state, product, model, device, transport_id});
      return acc;
    }, []);
  if (_.isEmpty(devices)) {
    log.debug('No connected devices have been detected');
